...
export function budgetBand(p: BuyerProfile): BudgetBand {
  const f = p.finances ?? {};
  const ownFunds = (f.deposit ?? 0) + (f.cash_buffer ?? 0) + (f.credit_line ?? 0);
  const loan = borrowingCapacity(p.income ?? 0, f.monthly_commitments ?? 0);
  ...
  return { floor, ceiling, borrowingCapacity: loan };
}
