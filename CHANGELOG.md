...
| 9 | `capabilities` oversold: "no commands to memorise" while the real output is gated behind a full profile; the 14-vs-1800 coverage split was buried; `what_it_can_do` and `coverage` contradicted each other on whether off-panel suburbs are scored | You ran `capabilities` live and called out the brochure-vs-reality gap | `start_here` was a sales line, not an honest path; coverage text predated `recommend_areas` and was never reconciled with it | Rewrote `start_here` (two steps, profile required, no useful one-word answer); made `coverage` a plain TIER 1 (curated, all criteria) vs TIER 2 (WA-wide, proximity+land+budget+live prices, no schools/growth/family) statement; reconciled `what_it_can_do` |
| 10 | Off-panel anchor (Mandurah) reported as "cannot be fit-scored, use nearby_suburbs + manual reads" | You pivoted the anchor to Mandurah; the model fell back to the curated-only path | Server instructions never routed off-panel anchors to `recommend_areas`; the `nearby_suburbs` description still claimed scoring was "being layered on next" (stale since M2) | Added explicit ANCHOR ROUTING to the instructions (off-panel anchor -> `recommend_areas`, which fit-scores it on proximity+land+budget against live prices); corrected the stale `nearby_suburbs` description |
| 11 | The `meets` flag was a baked, profile-less lie: `land: null` -> `meets: true`, beds ignored entirely, and it was computed at the 500sqm default not the buyer's filter (so a 520sqm listing read as meeting a 700sqm rule) | You ran `search_listings` with a 700sqm filter and spotted null-land marked meets and 520/524 marked meets while 457/461 were not | `meets` was precomputed and stored statically; `matchListings` had the null-land bug (`!L.land` treats unknown as a pass); `search_listings` surfaced the static flag despite having no profile | `matchListings` now computes `meets` LIVE from the resolved profile, null-safe (unknown land/beds/price is NOT a pass, with a plain-English note); `search_listings` strips the profile-less `meets`/`reason` and says criteria-fit is not evaluated without a profile; fixed the same null-land bug in `scripts/fetch_listings.py` and recomputed the committed sample; added `matchlistings.test.ts` (11 cases) as a regression guard |
| 12 | `search_listings` silently served the committed sample as if it might be live | You flagged "search listings quietly serving stale data" | The sample fallback note existed but was easy to miss, and the tool gave no profile-fit context | Kept the explicit `source: "sample"` note; clarified the tool is a raw curated-only feed and pointed criteria-fit to `match_listings` and off-panel anchors to `recommend_areas`. (Live data itself needs `RAPIDAPI_KEY` live on the deployment, which is the env/deploy step, not a code bug.) |
| 13 | No way to get INDIVIDUAL listings for an off-metro suburb with a hard land filter, so the model fell back to web-scraping portals for "Mandurah houses over 700sqm" | You watched the assistant resort to messy portal scraping when the engine had just fetched 20 live Mandurah listings per suburb | `match_listings` is metro-only (distance needs the curated set); `recommend_areas` returns suburb aggregates, not individual listings; nothing exposed per-listing results statewide with filters | Added `find_listings`: individual live houses for ANY WA suburb with HARD `min_land`/`min_beds`/`max_price` filters, built on the existing statewide `fetchSuburbListings`. Unknown land is dropped and reported as `unknownLand` (not silently passed). Pure filter extracted as `applyHardFilters` with `find-listings.test.ts` (9 cases). Verified live: RapidAPI returns 20 rows/suburb, correct `tieredResults` shape, parser keeps the houses |
